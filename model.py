...
class Multimodel(object):
    ...
    def encoder_maker(self, modality):
        ...
        conv = Conv2D(32, 3, 3, border_mode='same', name='enc_' + modality + '_conv1')(inp)
        act = LeakyReLU()(conv)
        conv = Conv2D(32, 3, 3, border_mode='same', name='enc_' + modality + '_conv2')(act)
        act1 = LeakyReLU()(conv)

        # downsample 1st level
        pool = MaxPooling2D(pool_size=(2, 2))(act1)
        conv = Conv2D(64, 3, 3, border_mode='same', name='enc_' + modality + '_conv3')(pool)
        act = LeakyReLU()(conv)
        conv = Conv2D(64, 3, 3, border_mode='same', name='enc_' + modality + '_conv4')(act)
        act2 = LeakyReLU()(conv)

        # downsample 2nd level
        pool = MaxPooling2D(pool_size=(2, 2))(act2)
        conv = Conv2D(128, 3, 3, border_mode='same', name='enc_' + modality + '_conv5')(pool)
        act = LeakyReLU()(conv)
        conv = Conv2D(128, 3, 3, border_mode='same', name='enc_' + modality + '_conv6')(act)
        act = LeakyReLU()(conv)

        # upsample 2nd level
        ups = UpSampling2D(size=(2, 2))(act)
        conv = Conv2D(64, 3, 3, border_mode='same', name='enc_' + modality + '_conv7')(ups)
        skip = merge([act2, conv], mode='concat', concat_axis=1, name='enc_' + modality + '_skip1')
        conv = Conv2D(64, 3, 3, border_mode='same', name='enc_' + modality + '_conv8')(skip)
        act = LeakyReLU()(conv)
        conv = Conv2D(64, 3, 3, border_mode='same', name='enc_' + modality + '_conv9')(act)
        act = LeakyReLU()(conv)

        # upsample 2nd level
        ups = UpSampling2D(size=(2, 2))(act)
        conv = Conv2D(32, 3, 3, border_mode='same', name='enc_' + modality + '_conv10')(ups)
        skip = merge([act1, conv], mode='concat', concat_axis=1, name='enc_' + modality + '_skip2')
        conv = Conv2D(32, 3, 3, border_mode='same', name='enc_' + modality + '_conv11')(skip)
        act = LeakyReLU()(conv)
        conv = Conv2D(32, 3, 3, border_mode='same', name='enc_' + modality + '_conv12')(act)
        act = LeakyReLU()(conv)

        conv_ld = self.latent_dim / 2 if self.common_merge == 'hemis' else self.latent_dim
        conv = Conv2D(conv_ld, 3, 3, border_mode='same', name='enc_' + modality + '_conv13')(act)
        lr = LeakyReLU()(conv)
    ...
    def decoder_maker(self, modality):
        ...
        conv = Conv2D(32, 3, 3, border_mode='same', activation='relu', name='dec_' + modality + '_conv1')(inp)
        conv = Conv2D(32, 3, 3, border_mode='same', activation='relu', name='dec_' + modality + '_conv2')(conv)
        skip = merge([inp, conv], mode='concat', concat_axis=1, name='dec_' + modality + '_skip1')
        conv = Conv2D(32, 3, 3, border_mode='same', activation='relu', name='dec_' + modality + '_conv3')(skip)
        conv = Conv2D(32, 3, 3, border_mode='same', activation='relu', name='dec_' + modality + '_conv4')(conv)
        skip = merge([skip, conv], mode='concat', concat_axis=1, name='dec_' + modality + '_skip2')
        conv = Conv2D(1, 1, 1, border_mode='same', activation='relu', name='dec_' + modality + '_conv5')(skip)
...
def new_flatten(emb, name=''):
    ...
    l = Lambda(lambda x: K.expand_dims(x, dim=1), name=name)(l)
    return l
